...
func ExecCommand(ctx context.Context, service *core.Service, command string) (string, error) {
	...
	case orderListCommandText:
		...
		if len(parts) == 6 {
			...
			if parts[3] == "instock" {
				limitPos = 4
			} else if parts[5] != "instock" {
				return "", errors.New("Некорректные дополнительные аргументы")
			}
			isInStock = true
			if parts[limitPos] != "limit" {
				return "", errors.New("Некорректные дополнительные аргументы")
			}
	...
	case newPvzCommandText:
		...
		_, err = service.ModifyPvz(ctx, pvz.Pvz{
			ID:       int64(pvzID),
...
func formOrder(o orders.Order) string {
	return fmt.Sprint(
